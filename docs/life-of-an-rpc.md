...
9. The interceptors then have a chance to do some more processing, each returning now in the opposite order. For us, `MonitoringInterceptor` logs the duration of the API call, and if the RPC failed removes any unhandled exceptions from the error and instead returns a generic "unknown error".
