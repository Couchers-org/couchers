...
const UseFontResizer = ({containerRef, textRef}: FontResizerProps ) => {
    useEffect(() => {
        if (containerRef.current && containerRef.current.offsetWidth && textRef.current && textRef.current.offsetWidth) {
            const scrollWidth = textRef.current.scrollWidth;
            const containerWidth = containerRef.current.offsetWidth;
            const oldFont = parseFloat(window.getComputedStyle(textRef.current, null).fontSize)
