...
export interface UpdatePrompt {
  info: NativeUpdateInfo.AsObject;
  mode: UpdateMode;
}

// Whether the backend is asking the user to do anything at all.
export function isActionable(info: NativeUpdateInfo.AsObject): boolean {
  return (
...
export function updateMode(
  ...
  const actByMs = info.actBy
    ? info.actBy.seconds * 1000 + info.actBy.nanos / 1e6
    : null;
  if (actByMs === null || actByMs <= now.getTime()) return "block";
  return "warn";
}
