...
// Detect if the native module is available (e.g., not available in Expo Go)
const hasAsyncStorage: boolean = Boolean(
  // iOS/Android module name in RN 0.7x
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (NativeModules as any)?.RNCAsyncStorage ||
    (NativeModules as any)?.RNC_AsyncStorage
);
