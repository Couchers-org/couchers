...
- **iOS (simulator)** → EAS builds a simulator `.app` (the `devtool-simulator` profile), which CI downloads and publishes to the dev-assets bucket at a stable URL: **`https://ios-simulator--devtool-builds.preview.couchershq.org/`**. For Mac devs who want to run the Dev Tool in the iOS Simulator without a local Xcode build. The page lists the `tar`/`xcrun simctl install` steps; re-download to update.
