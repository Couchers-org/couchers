...
# Fingerprints are pinned in app/mobile/fingerprints.json (verified on every
# branch by test:mobile-fingerprints), so this job just compares the file's
# value at HEAD against its value at the previous develop commit to decide
# whether to build. No build → EAS_BUILD_ID is left empty so submit becomes a
# no-op.
#
...
# the develop HEAD before this push. We compare against the prior file rather
# than re-running fingerprint:generate so the decision is based purely on the
...
# didn't exist yet" (the migration commit that introduced fingerprints.json —
# don't rebuild, since the file should reflect what's already deployed).
PREVIOUS=""
...
if [ "${FORCE_NATIVE_BUILD_AND_SUBMIT:-}" = "true" ]; then
  ...
  echo "fingerprints.json wasn't present at the previous develop commit — treating as unchanged (migration). Use FORCE_NATIVE_BUILD_AND_SUBMIT to override."
