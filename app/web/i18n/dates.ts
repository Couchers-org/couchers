...
export interface LocalizeDateTimeParams {
...
export function localizeDateTimeRange(
  ...
  const s = dayjs.isDayjs(start) ? start.toDate() : start;
  const e = dayjs.isDayjs(end) ? end.toDate() : end;
  const format = getIntlDateTimeFormat(args);
  const formatted = format.formatRange(s, e);
  return args.capitalize
    ? capitalizeFirstLetter(formatted, args.locale)
    : formatted;
}
