...
export default function AllMessagesTab() {
  ...
  const { ref: loadMoreRef } = useOnVisibleEffect(hasNextPage && !isFetchingNextPage ? fetchNextPage : undefined);
  ...
  const renderThread = (thread: MessageThread.AsObject) => {
    if (thread.groupChat) {
      return (
        ...
        >
          <StyledGroupChatListItem groupChat={thread.groupChat} isArchived={showArchived} />
        </Link>
      );
    }
    if (thread.hostRequest) {
      return (
        <Link
          ...
          onClick={guardMenuNavigation}
        >
          <StyledHostRequestListItem hostRequest={thread.hostRequest} isArchived={showArchived} />
        </Link>
      );
    }
    return null;
  ...
  const emptyStateText = showArchived ? t("archive.no_archived_messages") : t("all_messages_tab.no_messages");
  ...
  if (isGroupedView) {
    ...
    messagesContent = (
      <StyledList>
        {threads.length === 0 ? <TextBody>{emptyStateText}</TextBody> : threads.map(renderThread)}
        {hasNextPage && (
          <div ref={loadMoreRef}>
            <CenteredSpinner />
          </div>
