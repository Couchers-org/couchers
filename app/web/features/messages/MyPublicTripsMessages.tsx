...
import { ListMessageThreadsRes, MessageThreadCategory } from "proto/conversations_pb";
...
const StyledGroupHeader = styled("button")<{ expanded: boolean }>(({ theme, expanded }) => ({
  width: "100%",
  display: "flex",
  alignItems: "center",
...
export default function MyPublicTripsMessages() {
  ...
  const targetTripId = router.query.trip ? Number(router.query.trip) : undefined;
  ...
  const offers =
    offersQuery.data?.pages.flatMap((page) =>
      page.threadsList.map((thread) => thread.hostRequest).filter((hr): hr is HostRequest.AsObject => hr !== undefined),
    ) ?? [];
  ...
  return (
    <div>
      <StyledActions>
        <Button variant="outlined" size="small" startIcon={<EditIcon />} component={Link} href={myPublicTripsRoute}>
      ...
      {tripGroups.map((trip) => {
        ...
        const isDimmed =
          ...
          Temporal.PlainDate.compare(Temporal.PlainDate.from(trip.toDate), Temporal.Now.plainDateISO()) < 0;
        return (
          <StyledGroupContainer key={trip.tripId} ref={trip.tripId === targetTripId ? scrollTargetIntoView : undefined}>
            <StyledGroupHeader
            ...
            >
              ...
              <Place
                sx={{
                  color: isDimmed ? "var(--mui-palette-grey-500)" : "var(--mui-palette-primary-dark)",
                }}
              />
              <div style={{ flex: 1, minWidth: 0 }}>
                <Typography variant="h3" noWrap sx={isDimmed ? { color: "var(--mui-palette-grey-500)" } : undefined}>
                  {trip.communityName}
                </Typography>
                <Typography variant="body2" color="textSecondary">
                  ...
                  {t("my_public_trips.nights", {
                    count: Temporal.PlainDate.from(trip.toDate).since(Temporal.PlainDate.from(trip.fromDate)).days,
                  })}
                </Typography>
              </div>
