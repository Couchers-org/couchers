...
describe("Comment form", () => {
  ...
  it("renders the comment form successfully", async () => {
    ...
    expect(screen.getByRole("button", { name: t("communities:comment") })).toBeInTheDocument(); //can't check if visible, since this renders collapsed
  });
  ...
  it("submits valid comment without issue", async () => {
    ...
    const commentInput = (await screen.findByLabelText(t("communities:write_comment_a11y_label"))) as HTMLInputElement;
  ...
  it("cannot be submitted empty", async () => {
    ...
    expect(screen.getByLabelText(t("communities:write_comment_a11y_label"))).toBeEmptyDOMElement();
...
describe("Comment form profile-completeness gate", () => {
  ...
  it("shows the profile incomplete dialog instead of posting when the profile is incomplete", async () => {
    ...
    await user.click(screen.getByRole("button", { name: t("communities:comment") }));
  ...
  it("posts the comment as usual when the profile is complete", async () => {
    ...
    await waitFor(() => expect(postReplyMock).toHaveBeenCalledTimes(1));
    expect(screen.queryByLabelText(t("profile:complete_profile_dialog.title"))).not.toBeInTheDocument();
  });
});
