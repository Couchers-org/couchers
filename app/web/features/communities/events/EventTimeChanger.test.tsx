...
describe("Event time changer", () => {
  ...
  describe("auto-set end date behavior", () => {
    ...
    it("should auto-adjust end date when start date is changed to be after end date", async () => {
      ...
      // Now change start date to after end date
      await user.click(startDateGroup);
      await user.keyboard("{Control>}a{/Control}");
      await user.keyboard("08152021");

      // End date should be auto-adjusted to match new start date
      const startTimeGroup = screen.getByRole("group", {
        name: t("communities:start_time"),
      });
      await user.click(startTimeGroup);
      ...
      // End date should have been auto-adjusted to match new start date
      expect(submittedData.startDate.format("YYYY-MM-DD")).toBe("2021-08-15");
      expect(submittedData.endDate.format("YYYY-MM-DD")).toBe("2021-08-15");
    });

    it("should preserve end date when start date changes but end date is still valid", async () => {
      render(<TestForm />, { wrapper });
      ...
      // Set end date to later (multi-day event)
      const endDateGroup = screen.getByRole("group", {
        name: t("communities:end_date"),
      });
      await user.click(endDateGroup);
      ...
      // Change start date but keep it before end date
      await user.click(startDateGroup);
      await user.keyboard("{Control>}a{/Control}");
      await user.keyboard("08072021");
      ...
      // End date should be preserved (multi-day event)
      expect(submittedData.startDate.format("YYYY-MM-DD")).toBe("2021-08-07");
      expect(submittedData.endDate.format("YYYY-MM-DD")).toBe("2021-08-10");
    });
  });
});
