...
- Click "download artifacts" on the right. This is a copy of the repo, but it has the generated gRPC code in it, so you can copy that from `couchers/app/frontend/couchers-core/src/proto` to your local clone of the repo.

Then you need to target the dev preview api instead of localhost.

- In `couchers/app/frontend/couchers-core/.env.development`, change `REACT_APP_API_BASE_URL=http://localhost:8888` to `REACT_APP_API_BASE_URL=https://dev-api.couchershq.org`
- Remember not to commit this file to any pull requests!

### Then

You should then have the gRPC code in `couchers/app/frontend/couchers-core/src/proto`, and you can use the below `yarn` commands to run the web frontend.
