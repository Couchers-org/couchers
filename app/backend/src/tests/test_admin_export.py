...
def test_ExportUserData_conversations_with_invisible_user(db, moderator):
    ...
    (chat,) = export["group_chats"]
    assert [message["text"] for message in chat["messages"] if message.get("text")] == [
        "hello from me",
        "hello back from them",
    ]
    their_message = next(message for message in chat["messages"] if message.get("text") == "hello back from them")
    assert their_message["author"] == {"user_id": str(other.id)}
    assert chat["members"] == [{"user_id": str(user.id), "username": f"@{user.username}"}, {"user_id": str(other.id)}]

    assert export.get("host_requests", []) == []
...
def test_ExportUserData_covers_content_across_the_site(db, moderator):
    ...
    with threads_session(token) as api:
        ...
        api.PostReply(threads_pb2.PostReplyReq(thread_id=event.thread.thread_id, content="export event comment"))
    ...
    comments = {comment["content"]: comment["posted_in"] for comment in export["discussions"]["comments"]}
    assert comments["export comment"] == {"type": "discussion", "title": "export discussion"}
    assert comments["export event comment"] == {"type": "event", "title": "export event"}
...
def test_ExportUserData_for_a_banned_user_is_not_gutted(db, moderator):
    super_user, super_token = generate_user(is_superuser=True)
    user, token = generate_user()
    other, other_token = generate_user()
    ...
    with references_session(other_token) as api:
        reference_id = api.WriteFriendReference(
            references_pb2.WriteFriendReferenceReq(
                to_user_id=user.id, text="a good friend", was_appropriate=True, rating=0.5
            )
        ).reference_id
    moderator.approve_reference(reference_id)

    make_user_invisible(user.id)

    export = _export(super_token, user)

    assert export["friends"] == [
        {"user": {"user_id": str(other.id), "username": f"@{other.username}"}, "status": "friends"}
    ]

    (received,) = export["references"]["received"]
    assert received["text"] == "a good friend"
    assert received["from_user"] == {"user_id": str(other.id), "username": f"@{other.username}"}
    ...
    (request,) = export["host_requests"]
    assert request["host"] == {"user_id": str(other.id), "username": f"@{other.username}"}
    assert [message["text"] for message in request["messages"] if message.get("text")] == [
        valid_request_text(),
        "sure, come on over",
    ]


def test_ExportUserData_keeps_references_from_writers_who_deleted_their_account(db, moderator):
    super_user, super_token = generate_user(is_superuser=True)
    user, token = generate_user()
    other, other_token = generate_user()
    ...
    with session_scope() as session:
        session.execute(update(User).where(User.id == other.id).values(deleted_at=func.now()))

    (received,) = _export(super_token, user)["references"]["received"]
    assert received["text"] == "a good friend"
    assert received["from_user"] == {"user_id": str(other.id)}
