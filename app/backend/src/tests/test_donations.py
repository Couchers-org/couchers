...
def test_one_time_donation_flow(db, monkeypatch):
    ...
    with session_scope() as session:
        ...
        assert invoice.invoice_type == InvoiceType.donation
        assert (
...
def test_merch_invoice_flow(db, monkeypatch):
    """Test that merch purchases are correctly categorized as InvoiceType.merch"""
    user, token = generate_user()
    user_id = user.id

    with session_scope() as session:
        user_obj = session.execute(select(User).where(User.id == user_id)).scalar_one()
        user_obj.stripe_customer_id = "cus_Pv4uq0gT0rDZWN"
        session.commit()
    ...
    with session_scope() as session:
        invoice = session.execute(select(Invoice).where(Invoice.user_id == user_id)).scalar_one()
        assert invoice.invoice_type == InvoiceType.merch
        ...
        assert len(donation_notifications) == 0


def test_unknown_metadata_error(db, monkeypatch):
    """Test that webhooks with unknown metadata raise an error"""
    user, token = generate_user()
    user_id = user.id
    ...
    ## Stripe sends a charge.succeeded webhook with unknown metadata
    with pytest.raises(grpc.RpcError) as exc_info:
        fire_stripe_event("evt_unknown_metadata")

    assert "Unable to determine invoice_type" in str(exc_info.value.details())
    assert "pi_unknown_test_12345" in str(exc_info.value.details())
    assert "some_random_key" in str(exc_info.value.details())
