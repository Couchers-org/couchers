...
from couchers.notifications.utils import (
    ...
    get_topic_action_setting_description,
...
def test_topic_action_unsubscribe_texts() -> None:
    for topic_action in NotificationTopicAction:
        if not topic_action.is_critical:
            assert get_topic_action_unsubscribe_text(topic_action, locale=DEFAULT_LOCALE) is not None


def test_topic_key_unsubscribe_texts() -> None:
    for topic_action in NotificationTopicAction:
        if can_unsubscribe_topic_key(topic_action):
            assert get_topic_key_unsubscribe_text(topic_action, locale=DEFAULT_LOCALE) is not None
