...
def test_ListHostRequests(db, moderator):
    ...
    with requests_session(token2) as api:
        ...
        assert res.host_requests[0].status == conversations_types_pb2.HOST_REQUEST_STATUS_PENDING
...
def test_ListHostRequests_active_filter(db, moderator):
    ...
    with requests_session(token1) as api:
        api.RespondHostRequest(
            requests_pb2.RespondHostRequestReq(
                host_request_id=request_id, status=conversations_types_pb2.HOST_REQUEST_STATUS_CANCELLED
            )
        )
...
def test_ListHostRequests_status_in_filter(db, moderator):
    ...
    with requests_session(token2) as api:
        res = api.ListHostRequests(
            requests_pb2.ListHostRequestsReq(
                status_in=[conversations_types_pb2.HOST_REQUEST_STATUS_ACCEPTED],
            )
        )
        assert len(res.host_requests) == 1
        assert res.host_requests[0].status == conversations_types_pb2.HOST_REQUEST_STATUS_ACCEPTED

    # Filter to pending only
    with requests_session(token2) as api:
        res = api.ListHostRequests(
            requests_pb2.ListHostRequestsReq(
                status_in=[conversations_types_pb2.HOST_REQUEST_STATUS_PENDING],
            )
        )
        assert len(res.host_requests) == 1
        assert res.host_requests[0].status == conversations_types_pb2.HOST_REQUEST_STATUS_PENDING
    ...
    with requests_session(token2) as api:
        res = api.ListHostRequests(
            requests_pb2.ListHostRequestsReq(
                status_in=[
                    conversations_types_pb2.HOST_REQUEST_STATUS_ACCEPTED,
                    conversations_types_pb2.HOST_REQUEST_STATUS_PENDING,
                ],
            )
        )
        assert len(res.host_requests) == 2
    ...
    with requests_session(token2) as api:
        res = api.ListHostRequests(
            requests_pb2.ListHostRequestsReq(
                status_in=[conversations_types_pb2.HOST_REQUEST_STATUS_CONFIRMED],
            )
        )
        assert len(res.host_requests) == 0
...
def test_RespondHostRequests(db, moderator):
    ...
    with requests_session(token1) as api:
        with pytest.raises(grpc.RpcError) as e:
            api.RespondHostRequest(
                requests_pb2.RespondHostRequestReq(
                    host_request_id=request_id, status=conversations_types_pb2.HOST_REQUEST_STATUS_ACCEPTED
                )
            )
    ...
    with requests_session(token2) as api:
        ...
        with pytest.raises(grpc.RpcError) as e:
            api.RespondHostRequest(
                requests_pb2.RespondHostRequestReq(
                    host_request_id=9999, status=conversations_types_pb2.HOST_REQUEST_STATUS_CANCELLED
                )
            )
        assert e.value.code() == grpc.StatusCode.NOT_FOUND
        ...
        with pytest.raises(grpc.RpcError) as e:
            api.RespondHostRequest(
                requests_pb2.RespondHostRequestReq(
                    host_request_id=request_id, status=conversations_types_pb2.HOST_REQUEST_STATUS_CONFIRMED
                )
            )
        assert e.value.code() == grpc.StatusCode.PERMISSION_DENIED
        ...
        assert (
            res.messages[1].host_request_status_changed.status == conversations_types_pb2.HOST_REQUEST_STATUS_REJECTED
        )
    ...
    with requests_session(token1) as api:
        ...
        with pytest.raises(grpc.RpcError) as e:
            api.RespondHostRequest(
                requests_pb2.RespondHostRequestReq(
                    host_request_id=request_id, status=conversations_types_pb2.HOST_REQUEST_STATUS_PENDING
                )
            )
        assert e.value.code() == grpc.StatusCode.PERMISSION_DENIED
        assert e.value.details() == "You can't set the host request status to that."
    ...
    with requests_session(token1) as api:
        ...
        assert (
            res.messages[0].host_request_status_changed.status == conversations_types_pb2.HOST_REQUEST_STATUS_CANCELLED
        )
        assert (
            res.messages[1].host_request_status_changed.status == conversations_types_pb2.HOST_REQUEST_STATUS_CONFIRMED
        )
        assert (
            res.messages[2].host_request_status_changed.status == conversations_types_pb2.HOST_REQUEST_STATUS_ACCEPTED
        )
        assert (
            res.messages[4].host_request_status_changed.status == conversations_types_pb2.HOST_REQUEST_STATUS_REJECTED
        )
...
def test_get_host_request_messages(db, moderator):
    ...
    with requests_session(token2) as api:
        api.RespondHostRequest(
            requests_pb2.RespondHostRequestReq(
                host_request_id=conversation_id, status=conversations_types_pb2.HOST_REQUEST_STATUS_ACCEPTED
            )
        )
        ...
        api.RespondHostRequest(
            requests_pb2.RespondHostRequestReq(
                host_request_id=conversation_id, status=conversations_types_pb2.HOST_REQUEST_STATUS_REJECTED
            )
        )
    ...
    with requests_session(token1) as api:
        ...
        assert (
            res.messages[0].host_request_status_changed.status == conversations_types_pb2.HOST_REQUEST_STATUS_REJECTED
        )
        ...
        assert (
            res.messages[0].host_request_status_changed.status == conversations_types_pb2.HOST_REQUEST_STATUS_ACCEPTED
        )
        assert res.messages[0].WhichOneof("content") == "host_request_status_changed"
...
def test_SendHostRequestMessage(db, moderator):
    ...
    with requests_session(token2) as api:
        ...
        api.RespondHostRequest(
            requests_pb2.RespondHostRequestReq(
                host_request_id=host_request_id, status=conversations_types_pb2.HOST_REQUEST_STATUS_REJECTED
            )
        )
        api.SendHostRequestMessage(
    ...
    with requests_session(token1) as api:
        ...
        api.RespondHostRequest(
            requests_pb2.RespondHostRequestReq(
                host_request_id=host_request_id, status=conversations_types_pb2.HOST_REQUEST_STATUS_CANCELLED
            )
        )
        api.SendHostRequestMessage(
            requests_pb2.SendHostRequestMessageReq(host_request_id=host_request_id, text="Test message 3")
...
def test_get_updates(db, moderator):
    ...
    with requests_session(token1) as api:
        ...
        assert (
            res.messages[1].host_request_status_changed.status == conversations_types_pb2.HOST_REQUEST_STATUS_CANCELLED
        )
        ...
        assert (
            res.updates[1].message.host_request_status_changed.status
            == conversations_types_pb2.HOST_REQUEST_STATUS_CANCELLED
        )
        assert res.updates[1].status == conversations_types_pb2.HOST_REQUEST_STATUS_CANCELLED
        assert res.updates[2].message.text.text == "Test message 3"
        assert res.updates[3].message.WhichOneof("content") == "chat_created"
        assert res.updates[3].status == conversations_types_pb2.HOST_REQUEST_STATUS_PENDING
        ...
        assert res.updates[0].message.text.text == "Test message 2"
        assert res.updates[0].status == conversations_types_pb2.HOST_REQUEST_STATUS_CANCELLED
...
def test_archive_host_request(db, moderator):
    ...
    with requests_session(token1) as api:
        ...
        res = api.ListHostRequests(requests_pb2.ListHostRequestsReq(only_sent=True))
        assert len(res.host_requests) == 1
        assert res.host_requests[0].status == conversations_types_pb2.HOST_REQUEST_STATUS_CANCELLED
...
def test_quick_decline(db, email_collector: EmailCollector, push_collector: PushCollector, moderator):
    ...
    with requests_session(surfer_token) as api:
        ...
        assert res.status == conversations_types_pb2.HOST_REQUEST_STATUS_REJECTED
...
def test_host_req_feedback(db, moderator):
    ...
    with requests_session(host2_token) as api:
        api.RespondHostRequest(
            requests_pb2.RespondHostRequestReq(
                host_request_id=hr2_id, status=conversations_types_pb2.HOST_REQUEST_STATUS_REJECTED
            )
        )
    ...
    with requests_session(host3_token) as api:
        api.RespondHostRequest(
            requests_pb2.RespondHostRequestReq(
                host_request_id=hr3_id, status=conversations_types_pb2.HOST_REQUEST_STATUS_REJECTED
            )
        )
