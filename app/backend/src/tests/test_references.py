...
def create_host_request(
    session, surfer_id, host_id, host_request_age=timedelta(days=15), status=HostRequestStatus.confirmed
):
    """
    ...
    with requests_session(token1) as api:
        res = api.CreateHostRequest(
            requests_pb2.CreateHostRequestReq(
                host_id=user2.id, from_date=req_start, to_date=req_end, text="Test request"
            )
        )
