...
def test_list_message_threads(db):
    ...
    with api_session(db, token1) as api:
        ...
        res = api.CreateMessageThread(api_pb2.CreateMessageThreadReq(
        ...
        res = api.ListMessageThreads(
            api_pb2.ListMessageThreadsReq(start_index=1, max=1))
        assert len(res.threads) == 1
...
def test_make_remove_message_thread_admin(db):
    ...
    with api_session(db, token2) as api:
        ...
        api.RemoveMessageThreadAdmin(
            api_pb2.RemoveMessageThreadAdminReq(thread_id=thread_id, user="user2"))
...
def test_send_message(db):
    ...
    with api_session(db, token1) as api:
        ...
        api.SendMessage(api_pb2.SendMessageReq(
            thread_id=res.thread_id, message="Test message 1"))
        res = api.GetMessageThread(
            api_pb2.GetMessageThreadReq(thread_id=res.thread_id))
        assert res.messages[0].text == "Test message 1"
