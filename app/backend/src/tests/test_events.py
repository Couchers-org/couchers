...
def test_DuplicateEvent(db):
    # Test cases:
    # 1. Event creator can duplicate their event
    # 2. Photo is copied from original event when no new photo provided
    # 3. New photo overrides original when provided
    # 4. Non-creator cannot duplicate event (permission check on photo)
    # 5. Duplicated event has new dates but same content
    ...
    # Create original event with photo
    with events_session(token1) as api:
        # First create a mock photo upload
        from couchers.models import Upload

        with session_scope() as session:
            upload = Upload(key="test_photo_key", filename="test.jpg", creator_user_id=user1.id)
            session.add(upload)
            session.commit()

        original_event = api.CreateEvent(
            events_pb2.CreateEventReq(
                title="Original Event",
                content="Original content.",
                photo_key="test_photo_key",
                offline_information=events_pb2.OfflineEventInformation(
                    address="Near Null Island",
                    lat=0.1,
                    lng=0.2,
        ...
        assert original_event.photo_url  # Verify original has photo

    # Test 1: Creator can duplicate with photo copied (no new photo provided)
    new_start_time = now() + timedelta(hours=10)
    new_end_time = new_start_time + timedelta(hours=3)

    with events_session(token1) as api:
        duplicated_event = api.CreateEvent(
            events_pb2.CreateEventReq(
                title="Duplicated Event",
                content="Original content.",  # Same content
                # No photo_key provided - should copy from original
                offline_information=events_pb2.OfflineEventInformation(
                    address="Near Null Island",
                    lat=0.1,
                    lng=0.2,
                ...
                end_time=Timestamp_from_datetime(new_end_time),
                timezone="UTC",
                duplicate_from_event_id=original_event_id,
            )
        )

        assert duplicated_event.event_id != original_event_id
        assert duplicated_event.title == "Duplicated Event"
        assert duplicated_event.content == "Original content."
        assert duplicated_event.photo_url  # Photo should be copied
        assert duplicated_event.photo_url == original_event.photo_url  # Same photo
        assert to_aware_datetime(duplicated_event.start_time) == new_start_time
        assert to_aware_datetime(duplicated_event.end_time) == new_end_time

    # Test 2: New photo overrides original
    with events_session(token1) as api:
        # Create another mock photo upload
        with session_scope() as session:
            new_upload = Upload(key="new_photo_key", filename="new.jpg", creator_user_id=user1.id)
            session.add(new_upload)
            session.commit()

        duplicated_with_new_photo = api.CreateEvent(
            events_pb2.CreateEventReq(
                title="Duplicated with New Photo",
                content="Original content.",
                photo_key="new_photo_key",  # New photo provided
                offline_information=events_pb2.OfflineEventInformation(
                    address="Near Null Island",
                    lat=0.1,
                    lng=0.2,
        ...
        assert duplicated_with_new_photo.photo_url
        assert duplicated_with_new_photo.photo_url != original_event.photo_url  # Different photo

    # Test 3: Non-creator trying to duplicate doesn't get photo copied
    # (photo_key from original event is only accessible to creator)
    with events_session(token2) as api:
        duplicated_by_other = api.CreateEvent(
            events_pb2.CreateEventReq(
                title="Duplicated by Other",
                content="Different content.",
                # No photo_key provided
                offline_information=events_pb2.OfflineEventInformation(
                    address="Near Null Island",
                    lat=0.1,
                    lng=0.2,
        ...
        # Should not have photo because user2 is not the creator
        assert not duplicated_by_other.photo_url

    # Test 4: Duplicate from non-existent event should fail
    with events_session(token1) as api:
        with pytest.raises(grpc.RpcError) as e:
            api.CreateEvent(
                events_pb2.CreateEventReq(
                    title="Duplicate from Nowhere",
                    content="Content.",
                    offline_information=events_pb2.OfflineEventInformation(
                        address="Near Null Island",
                        lat=0.1,
                    ...
                    end_time=Timestamp_from_datetime(new_end_time),
                    timezone="UTC",
                    duplicate_from_event_id=99999,  # Non-existent
                )
            )
        assert e.value.code() == grpc.StatusCode.NOT_FOUND
        assert e.value.details() == "Source event not found."
