        ...
        with session_scope() as session:
            for city in cities:
                ...
                for usernumber in range(0, usersforthiscity):
                    ...
                    username = names.get_full_name(gender.lower())
                    userlat = citylat + (random.random() - .5) / 10
                    userlong = citylong + (random.random() - .5) / 10
                    new_user = User(
                        username=username.lower().replace(' ', '')+str(random.random()),
                        email=username.lower().replace(' ', '') +'_'+str(usernumber)+'_'+str(city[10])+ '@couchers.org',
                        hashed_password=hash_password(username[0]),
                        name=username,
                        city=city[0]+', '+city[4],
                        ...
                        color=random.choice(['red','blue','white',None]),
