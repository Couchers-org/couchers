...
class Requests(requests_pb2_grpc.RequestsServicer):
    ...
    def ListHostRequests(self, request, context):
        ...
        with session_scope(self._Session) as session:
            pagination = (request.number if request.number >
                          0 else PAGINATION_LENGTH)
            ...
            next_request_id = min(map(
                lambda g: g.HostRequest.id if g.HostRequest else 1, results))-1 if len(results) > 0 else 0  # TODO
        ...
        with session_scope(self._Session) as session:
            ...
            if not no_more:
                ...
                events_query = events_query.filter(
                    HostRequestEvent.after_message_id >= messages[-2].id)
