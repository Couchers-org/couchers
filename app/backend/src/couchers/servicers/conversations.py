...
from couchers.proto import conversations_pb2, conversations_pb2_grpc, conversations_types_pb2, notification_data_pb2
...
def _message_to_pb(message: Message) -> conversations_types_pb2.Message:
    """
    ...
    if message.is_normal_message:
        ...
        return conversations_types_pb2.Message(
            ...
            chat_edited=(
                conversations_types_pb2.MessageContentChatEdited()
                if message.message_type == MessageType.chat_edited
                else None
            ),
            user_invited=(
                conversations_types_pb2.MessageContentUserInvited(target_user_id=message.target_id)
                if message.message_type == MessageType.user_invited
                else None
            ),
            user_left=(
                conversations_types_pb2.MessageContentUserLeft()
                if message.message_type == MessageType.user_left
                else None
            ),
            user_made_admin=(
                conversations_types_pb2.MessageContentUserMadeAdmin(target_user_id=message.target_id)
                if message.message_type == MessageType.user_made_admin
                else None
            ),
            user_removed_admin=(
                conversations_types_pb2.MessageContentUserRemovedAdmin(target_user_id=message.target_id)
                if message.message_type == MessageType.user_removed_admin
                else None
            ),
            group_chat_user_removed=(
                conversations_types_pb2.MessageContentUserRemoved(target_user_id=message.target_id)
