...
class Stripe(stripe_pb2_grpc.StripeServicer):
    def Webhook(self, request, context):
        ...
        # For one-off donations, we only get one event, a `checkout.session.completed` when it completes.
        # For recurring donations, ... TODO
            ...
            if data_object["payment_intent"]:
                with session_scope() as session:
                    ...
                    if data_object["payment_status"] == "paid":
                        donation.paid = now()
                        # TODO: send a "thanks for a one off donation" email
                    else:
                ...
                with session_scope() as session:
                    ...
                    # TODO: send a "thank you for setting up a recurring donation" email
            else:
            ...
            with session_scope() as session:
                ...
                session.add(
                    Invoice(
                        ...
                        stripe_receipt_url=invoice_data["receipt_url"],
                    )
                )
                # TODO: send a "thank you for your donation" email
        else:
