...
from couchers.helpers.group_chats import was_subscribed_at
...
from couchers.sql import users_visible, where_moderated_content_visible_to_user_column, where_users_column_visible
...
def _string(value: str | None) -> StringValue | None:
    return StringValue(value=value) if value is not None else None


class _UserRefs:
    def __init__(self, session: Session, user: User, context: CouchersContext):
    ...
    def prefetch(self, user_ids: Iterable[int | None]) -> None:
        ...
        found: dict[int, str] = {
            ...
            for row in self.session.execute(
                select(User.id, User.username).where(User.id.in_(missing)).where(users_visible(self.context, User))
            )
        }
    ...
    def get_one(self, user_id: int) -> user_data_export_pb2.UserRef:
        username = self._username(user_id)
        return user_data_export_pb2.UserRef(user_id=user_id, username=_string(f"@{username}" if username else None))

    def get(self, user_id: int | None) -> user_data_export_pb2.UserRef | None:
        return self.get_one(user_id) if user_id is not None else None
...
def _references(
    session: Session, user: User, refs: _UserRefs, context: CouchersContext
) -> user_data_export_pb2.References:
    written = list(
    ...
    received_statement = where_reference_user_visible(received_statement, context, Reference.from_user_id)
    received = list(
...
def _host_requests(
    session: Session, user: User, refs: _UserRefs, context: CouchersContext
) -> list[user_data_export_pb2.HostRequest]:
    statement = where_moderated_content_visible_to_user_column(select(HostRequest), HostRequest, user.id)
    statement = where_users_column_visible(
        statement,
        context,
        case((HostRequest.surfer_user_id == user.id, HostRequest.host_user_id), else_=HostRequest.surfer_user_id),
    )
    host_requests = list(
        session.execute(
    ...
    for host_request in host_requests:
        ...
        out.append(
            user_data_export_pb2.HostRequest(
                ...
                messages=[_chat_message(message, refs) for message in messages],
            )
        )
    return out
...
def _group_chats(session: Session, user: User, refs: _UserRefs) -> list[user_data_export_pb2.GroupChat]:
    ...
    members_by_chat: dict[int, list[int]] = {}
    for chat_id, chat_subscriptions in subscriptions_by_chat.items():
        if chat_id not in visible_chat_ids:
            continue
        latest_subscription = chat_subscriptions[-1]
        members_statement = select(GroupChatSubscription.user_id).where(GroupChatSubscription.group_chat_id == chat_id)
        if latest_subscription.left is not None:
            members_statement = members_statement.where(
                was_subscribed_at(GroupChatSubscription, latest_subscription.left)
            )
        else:
            members_statement = members_statement.where(GroupChatSubscription.left.is_(None))
        members_by_chat[chat_id] = list(session.execute(members_statement.order_by(GroupChatSubscription.id)).scalars())
    refs.prefetch(member_id for members in members_by_chat.values() for member_id in members)
    ...
    for chat_id, chat_subscriptions in sorted(subscriptions_by_chat.items()):
        ...
        out.append(
            user_data_export_pb2.GroupChat(
                ...
                members=[refs.get_one(member_id) for member_id in members_by_chat.get(chat_id, [])],
                messages=[_chat_message(message, refs) for message in in_window],
            )
        )
    return out
...
def _thread_contexts(session: Session, thread_ids: set[int]) -> dict[int, user_data_export_pb2.ThreadContext]:
    ...
    for thread_id, title in session.execute(
        select(EventOccurrence.thread_id, Event.title)
        .join(Event, Event.id == EventOccurrence.event_id)
        .where(EventOccurrence.thread_id.in_(thread_ids))
    ):
...
def generate_user_data_export(session: Session, user: User, generated: datetime) -> user_data_export_pb2.UserDataExport:
    ...
    refs = _UserRefs(session, user, context)
    ...
    return user_data_export_pb2.UserDataExport(
        ...
        references=_references(session, user, refs, context),
        host_requests=_host_requests(session, user, refs, context),
