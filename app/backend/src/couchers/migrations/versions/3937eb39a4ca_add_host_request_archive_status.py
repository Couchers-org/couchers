...
def upgrade():
    op.add_column(
        "host_requests", sa.Column("is_host_archived", sa.Boolean(), server_default=sa.text("0"), nullable=False)
    )
    op.add_column(
        "host_requests", sa.Column("is_surfer_archived", sa.Boolean(), server_default=sa.text("0"), nullable=False)
    )
