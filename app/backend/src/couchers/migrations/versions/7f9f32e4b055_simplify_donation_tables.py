...
def upgrade():
    ...
    op.execute(
        """
        INSERT INTO donation_initiations (created, user_id, amount, stripe_checkout_session_id, donation_type)
        SELECT created, user_id, amount, stripe_checkout_session_id, 'one_time'
        FROM one_time_donations;

        INSERT INTO donation_initiations (created, user_id, amount, stripe_checkout_session_id, donation_type)
        SELECT created, user_id, amount, stripe_checkout_session_id, 'recurring'
        FROM recurring_donations;

        UPDATE donation_initiations
        SET id = t.new_id
        FROM (
            SELECT id, ROW_NUMBER() OVER (ORDER BY created) AS new_id
            FROM donation_initiations
        ) AS t
        WHERE donation_initiations.id = t.id;
        """
    )
