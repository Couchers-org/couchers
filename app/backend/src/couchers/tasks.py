...
def send_signup_email(email, token, expiry_text):
    ...
    emails.send_email(email, "Finish signing up for Couchers.org", "signup", template_args={"signup_link": signup_link})
...
def send_login_email(user, token, expiry_text):
    ...
    emails.send_email(user.email, "Your login link for Couchers.org", "login", template_args={"user": user, "login_link": login_link})
