...
mkdir -p frontend/couchers-core/src/proto/
...
find proto -name '*.proto' | protoc -I proto \
  ...
  \
  --js_out="import_style=commonjs,binary:frontend/couchers-core/src/proto" \
  --grpc-web_out="import_style=commonjs+dts,mode=grpcweb:frontend/couchers-core/src/proto" \
  \
  $(xargs)
